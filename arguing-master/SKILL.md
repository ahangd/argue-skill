---
name: arguing-master
description: Sharp argument and anti-troll reply craft for Chinese-language debates, comments, group chats, workplace conflicts, family discussions, and social media disputes. Use when the user wants to respond to bad-faith arguing, sophistry, strawmen, emotional manipulation, status flexing, fake kindness, topic shifting, personal attacks, or "杠精" behavior with logic, evidence, effortless expertise, cutting sarcasm, concise rebuttals, high-pressure clapbacks, or graceful exit lines while avoiding slurs, threats, doxxing, and uncontrolled escalation.
---
...
4. If the opponent flexes status, show effortless authority with one accurate detail instead of directly claiming superiority.
5. Use short sentences at the moment of impact.
6. Offer 3-5 reply options with visibly different intensity levels.
7. Include an exit line when further arguing is unlikely to help.
...
- Demonstrate, do not declare: use a natural concrete detail that proves familiarity without saying "我更懂".
...
When the opponent uses travel, taste, education, industry access, luxury consumption, or elite names to rank the user, prefer "show, don't claim." The strongest reply casually reveals familiarity, then moves back to the opponent's shallow frame.

Use this pattern:

```text
先随手给一个准确细节，再轻轻评价对方的框架。
```

Good expertise details are:

- Specific but not encyclopedic.
- Natural enough to sound lived-in.
- Relevant to the opponent's flex.
- Verifiable or phrased conditionally if the user's real experience is unknown.
- Followed by a frame refusal, not a long resume.

Avoid fake autobiography. Do not invent that the user has been somewhere, met someone, bought something, or holds a credential. If the user's experience is unknown, write options like "如果你确实熟悉巴黎，可以说..." or use impersonal knowledge: "懂巴黎的人一般不会只拿巴黎当奢侈品背景板。"

Templates:

```text
巴黎如果只剩铁塔和香街，那基本还是游客滤镜。你要真聊城市，我倒更想听你怎么理解左岸、玛黑和那些不靠打卡存在的小街区。
```

```text
伦敦如果只聊大本钟和牛津街，就跟来上海只聊外滩差不多。可以，但别急着拿它当见识。
```

```text
我对巴黎伦敦不陌生，所以更知道真正熟的人通常不会把城市名字当奖章挂出来。
```

```text
你说巴黎伦敦没问题，问题是你说得像刚从旅行软件首页学会这两个地名。
```

```text
如果你说的是某条街、某个区、某家店、某段路线，我可能还有兴趣聊；只报两个城市名就开始判断别人见没见过世面，这个精度有点低。
```

```text
我省钱不代表没出去过，可能只是不会把一次旅行拆成三个月的优越感分期。
```
...
For an indirect expertise kill, do not say "我比你懂巴黎伦敦"; reveal it:

```text
巴黎伦敦我不陌生，所以更知道真正熟的人一般不会只拿城市名做优越感。你要聊路线、街区、展馆、餐馆，我都可以聊；但你现在聊的是阶层感，不是旅行。
```

```text
下次真想约可以直接约。巴黎不是只有铁塔和香街，伦敦也不是只有大本钟和牛津街；如果只拿两个城市名判断别人见没见过世面，那你这趟可能主要逛的是优越感。
```

```text
我省钱是消费习惯，不是没见过世面。巴黎左岸、玛黑，伦敦 South Bank、Shoreditch 这些地方聊起来都行；但把城市当奖章挂出来，就有点像刚通关新手村。
```
